...
fn set_tile_applies() {
    ...
    h.apply(
        &mut level,
        EditorCommand::SetTile { room: 0, x: 2, y: 3, metatile: new },
    )
    .unwrap();
...
fn undo_restores_previous_value() {
    ...
    h.apply(&mut level, EditorCommand::SetTile { room: 0, x: 1, y: 1, metatile: before + 1 })
        .unwrap();
...
fn new_edit_after_undo_clears_redo() {
    let mut level = synthetic_level();
    let mut h = EditorHistory::new();
    h.apply(&mut level, EditorCommand::SetTile { room: 0, x: 0, y: 0, metatile: 1 }).unwrap();
    h.undo(&mut level);
    assert!(h.can_redo());
    h.apply(&mut level, EditorCommand::SetTile { room: 0, x: 0, y: 0, metatile: 2 }).unwrap();
    assert!(!h.can_redo());
}
...
fn move_object_round_trips_through_undo_redo() {
    ...
    h.apply(&mut level, EditorCommand::MoveObject { room: 0, object: 0, x: ox + 8, y: oy + 8 })
        .unwrap();
    assert_eq!(level.rooms[0].objects[0].x, ox + 8);
    h.undo(&mut level);
    assert_eq!((level.rooms[0].objects[0].x, level.rooms[0].objects[0].y), (ox, oy));
    h.redo(&mut level);
    assert_eq!((level.rooms[0].objects[0].x, level.rooms[0].objects[0].y), (ox + 8, oy + 8));
}
...
fn dirty_state_tracks_save_point() {
    ...
    h.undo(&mut level);
    assert!(h.is_dirty(), "undoing past the save point makes the document dirty");

    h.redo(&mut level);
    assert!(!h.is_dirty(), "redoing back to the save point makes it clean again");
}
...
fn command_on_invalid_target_errors_and_changes_nothing() {
    ...
    assert!(h
        .apply(&mut level, EditorCommand::SetTile { room: 99, x: 0, y: 0, metatile: 0 })
        .is_err());
    assert!(h
        .apply(&mut level, EditorCommand::SetTile { room: 0, x: 9999, y: 0, metatile: 0 })
        .is_err());
    assert!(h
        .apply(&mut level, EditorCommand::MoveObject { room: 0, object: 9999, x: 0, y: 0 })
        .is_err());
...
fn selection_transitions() {
    ...
    sel = Selection::Tile { room: 0, x: 3, y: 4 };
    assert!(sel.is_some());
